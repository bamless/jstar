...
static int argparse_getvalue(struct argparse *self, const struct argparse_option *opt, int flags) {
    ...
    case ARGPARSE_OPT_INTEGER:
        ...
        if(s[0] != '\0') argparse_error(self, opt, "expects an integer value", flags);
        break;
    case ARGPARSE_OPT_FLOAT:
        ...
        if(errno) argparse_error(self, opt, strerror(errno), flags);
        if(s[0] != '\0') argparse_error(self, opt, "expects a numerical value", flags);
        break;
    default:
        assert(0);
    }
...
int argparse_init(struct argparse *self, struct argparse_option *options, const char *const *usages,
                  int flags) {
...
void argparse_usage(struct argparse *self) {
    if(self->usages) {
        ...
        while(*self->usages && **self->usages) fprintf(stdout, "   or: %s\n", *self->usages++);
    } else {
