...
static _WDIR *_wopendir(const wchar_t *dirname) {
    ...
    if(dirp->patt == NULL) goto exit_closedir;
    ...
    *p = '\0';
    ...
    if(!dirent_first(dirp)) goto exit_closedir;
...
static WIN32_FIND_DATAW *dirent_first(_WDIR *dirp) {
    ...
    if(dirp->handle == INVALID_HANDLE_VALUE) goto error;
...
static WIN32_FIND_DATAW *dirent_next(_WDIR *dirp) {
    ...
    if(dirp->handle == INVALID_HANDLE_VALUE) return NULL;

    /* Read the next directory entry from stream */
    if(FindNextFileW(dirp->handle, &dirp->data) == FALSE) goto exit_close;
...
static DIR *opendir(const char *dirname) {
    ...
    if(error) goto exit_failure;

    /* Open directory stream using wide-character name */
    dirp->wdirp = _wopendir(wname);
    if(!dirp->wdirp) goto exit_failure;
...
static int readdir_r(DIR *dirp, struct dirent *entry, struct dirent **result) {
    ...
    if(error && datap->cAlternateFileName[0] != '\0') {
        error = wcstombs_s(&n, entry->d_name, PATH_MAX + 1, datap->cAlternateFileName,
                           PATH_MAX + 1);
    }
...
static int scandir(const char *dirname, struct dirent ***namelist,
    ...
    while(1) {
        ...
        struct dirent *entry;
        if(readdir_r(dir, tmp, &entry) != /*OK*/ 0) goto exit_failure;

        /* Stop if we already read the last directory entry */
        if(entry == NULL) goto exit_success;
...
static int dirent_mbstowcs_s(size_t *pReturnValue, wchar_t *wcstr, size_t sizeInWords,
    ...
    if(wcstr && n >= count) return /*error*/ 1;
...
static int dirent_wcstombs_s(size_t *pReturnValue, char *mbstr, size_t sizeInBytes,
    ...
    if(mbstr && n >= count) return /*error*/ 1;
